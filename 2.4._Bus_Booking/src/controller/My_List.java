...
public class My_List<T extends Comparable<T>>{
    ...
    void addToHead(int x) {
        NodeBooking a = new NodeBooking(x, head);
        head = a;
    ...
    //2
    void addToTail(T x) {
        NodeBooking a = new NodeBooking(x, null);
        if(isEmpty()) {
            head = tail = a;
            return;
        }
...
//    }
    ...
    void dele(T x) {
        ...
        NodeBooking temp = a.getNext();
        h.setNext(temp);
        a = null;
    }
    // 9
    NodeBooking search(T x){
        NodeBooking a = head;
        while(a != null){
            if(a.getData() == x){
                return a;
...
//    //12
     public void sort() {
        ...
        NodeBooking<T> current = head;
        NodeBooking<T> index = null;
        T temp;
    ...
    public void addBooking(BinarySearchTree<Bus> BST, List_Customer customerList) {
        String bcode;
        String ccode;
        while (true) {            
            bcode = Validation.getBcode("enter bcode", BST, customerList);
            ccode = Validation.getCcode("enter ccode", BST, customerList);
            if(!checkExist(bcode, ccode)){
                break;
            }
            System.out.println("duplicate bcode and ccode");
        }
    ...
    private boolean checkExist(String bcode, String ccode) {
        NodeBooking a = head;
        if(a == null){
            return false;
        }
        ...
        while(a!= null){
            if(((Booking)a.getData()).getBcode().equalsIgnoreCase(bcode)&&((Booking)a.getData()).getCcode().equalsIgnoreCase(ccode)){
                return true;
            }
            a = a.getNext();
        }
